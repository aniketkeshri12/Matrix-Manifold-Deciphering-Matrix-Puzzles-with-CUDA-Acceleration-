...
void CalculateHadamardProduct(long int* A, long int* B, int N) {
    ...
    unsigned idx = (blockIdx.x * blockDim.x) + threadIdx.x;// finding particular index position
    unsigned i = idx / N; //finding ith position
    unsigned j = idx % N; //finding jth position
    if (idx < N * N)
    {
        A[idx] = A[idx] * B[j * N + i]; //for B changing [i][j] to [j][i] to make it transpose multiplication
    }
}
...
void FindWeightMatrix(long int* A, long int* B, int N) {
    ...
    // TODO: Write your kernel here
    unsigned idx = (blockIdx.x * blockDim.x * blockDim.y) + (threadIdx.x * blockDim.y) + threadIdx.y;// finding particular index position
    if (idx < N * N) //checking range
    {
        if (A[idx] < B[idx]) // checking corresponding value
        {
            A[idx] = B[idx];
        }
    }
...
void CalculateFinalMatrix(long int* A, long int* B, int N) {
    ...
    // TODO: Write your kernel here
    //mapping D matrix 2*N*2*N kernel to N*N of matrix A to make more parallel
    unsigned bidx = (blockIdx.x * gridDim.y) + blockIdx.y;
    unsigned idx  = (bidx * blockDim.x * blockDim.y) + (threadIdx.x * blockDim.y) + threadIdx.y;
    if (idx < 2 * N * 2 * N) // checking range
    {
        unsigned i = idx / (2*N);  // finding ith and jth to map 
        unsigned j = idx % (2*N);

        unsigned idy = (i % N)*N + j%N; //generating new index for mapped matrix 
        B[idx] = A[idy] * B[idx];
    }
}
...
int main(int argc, char** argv) {
    ...
    */
    ...
    //Writing my own code allocating memory
    ...
    // writing my own code Copy input data from host to device
    ...
    cudaMemcpy(d_D, D, N * 2 * 2 * N * sizeof(long int), cudaMemcpyHostToDevice);
